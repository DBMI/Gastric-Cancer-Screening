``
--  select * from INSTITUTES_SILVER_DDIHSNI_DEV.WAREHOUSE.OMOP_STUDY
create or replace TABLE INSTITUTES_SILVER_DDIHSNI_DEV.WAREHOUSE.OMOP_STUDY AS
with PATIENT_GC_LIST as
(select *
FROM
(
select   a.Person_id as STUDY_PAT_ID,  CONCEPT_NAME ,CONCEPT_CODE, condition_start_date INITIAL_DX_DT ,BIRTH_DATETIME
        ,GENDER_SOURCE_VALUE ,race_source_value , ethnicity_source_value , f.state
...
from
(
select c.*, FIRST_ENC_DT, a.visit_start_date PCP_DT_PRIOR_TO_DX  , a.visit_source_value as study_visit_source, b.specialty_source_value as study_Specialty
        ,row_number() over (partition by a.person_id order by a.visit_start_date desc) as rn2
from  CDM_SILVER_OMOP_PRD.OMOP.VISIT_OCCURRENCE a
INNER JOIN CDM_SILVER_OMOP_PRD.OMOP.PROVIDER b 
...
)x where rn2 = 1 and INITIAL_DX_DT >= '2010-01-01'


/************************** CONTROLS *****************************/

--  select  * from INSTITUTES_SILVER_DDIHSNI_DEV.WAREHOUSE.GASTRIC_CA_STUDY_CONTROL 
create or replace table INSTITUTES_SILVER_DDIHSNI_DEV.WAREHOUSE.GASTRIC_CA_STUDY_CONTROL as
with CONTROLS as
(select *
from
(
select top 1000000 c.person_id  as Study_Person_ID
...
from
(
select STUDY_PAT_ID,  PCP_DT_PRIOR_TO_DX , GENDER_SOURCE_VALUE, race_source_value, ethnicity_source_value, state, 'STUDY'
...
select Study_person_id ,visit_start_date,  CONTROL_GENDER ,CONTROL_RACE , CONTROL_ETHNICITY,CONTROL_STATE  ,'CONTROL','1'
...
(select *
from
(select a.study_pat_id, b.measurement_date as BMI_DT , b.value_as_number as BMI
...
where  measurement_source_value ilike '%BODY MASS INDEX%'
...
(select *
from
(select a.study_pat_id, b.measurement_date as WEIGHT_DT , b.value_as_number as WEIGHT
        ,row_number() over (partition by a.study_pat_id order by b.measurement_date desc) as rn
from INSTITUTES_SILVER_DDIHSNI_DEV.WAREHOUSE.OMOP_COHORT a 
INNER JOIN CDM_SILVER_OMOP_PRD.OMOP.MEASUREMENT b on a.study_pat_id = b.person_id
where  measurement_source_value = 'WEIGHT' 
and cast(b.measurement_date as date) <= cast(PCP_DT_PRIOR_TO_DX as date)
and  b.value_as_number is not null
)x where rn = 1)
,ALCOHOL as
(select *
from
(select a.study_pat_id , b.observation_date as ALCOHOL_DT ,observation_source_value as ALCOHOL
...
where observation_source_value ilike '%ALCOHOL%'  and value_as_string = 'Yes'
and cast(b.observation_date as date) <= cast(PCP_DT_PRIOR_TO_DX as date)
)x where rn = 1)
,TOBACCO as
(select *
from
(select a.study_pat_id , b.observation_date as TOBACCO_DT ,observation_source_value as TOBACCO
        ,row_number() over (partition by a.study_pat_id order by b.observation_date desc) as rn
from INSTITUTES_SILVER_DDIHSNI_DEV.WAREHOUSE.OMOP_COHORT a 
INNER JOIN CDM_SILVER_OMOP_PRD.OMOP.OBSERVATION b on a.study_pat_id = b.person_id
where observation_source_value ilike '%TOBACCO%'  and value_as_string = 'Yes'
and cast(b.observation_date as date) <= cast(PCP_DT_PRIOR_TO_DX as date)
)x where rn = 1)
,FAM_HX_CA as
(select *
